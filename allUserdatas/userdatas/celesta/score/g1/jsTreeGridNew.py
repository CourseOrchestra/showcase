...
def getSetting(context, main, add, filterinfo, session, elementId):
    ...
    res = JythonDTO(None, settings, UserMessageFactory().build(555, u"Грид (Tree, metadata) успешно построен из Celesta"))
    return res

def getData(context, main, add, filterinfo, session, elementId, sortColumnList, firstrecord, pagesize, parentId):
